...
状态：阶段 0 至阶段 7 的当前交付批次已完成。阶段 1 至阶段 5 已交付广泛的 2D/UI 场景、动画、物理、导航、TileMap、视觉、粒子与控件语义能力，以及受策略约束的项目 2D Resource 查询、创建、属性更新、保存和按资源隔离的撤销/重做；阶段 6 已交付场景启动/停止、游戏日志、真实运行时截图、键盘/鼠标/多点触摸输入模拟、项目 Input Map 动作与键盘/鼠标/手柄绑定编辑、独立 `Shortcut` 资源及其输入事件序列编辑、`AudioStreamPlayer2D` 运行时状态/播放/停止/定位控制、受控 CanvasItem 运行时 Tween、受限性能采样、客户端 PNG 内容断言和有总超时/自动清理边界的游戏测试编排；阶段 7 已交付运行时 `ClassDB` 2D 覆盖审计、完整快照、跨版本差异、类型详情反射和全允许节点生命周期验收；`OptionButton`/`MenuButton` 平面菜单写入已收紧为 Godot 场景真实可持久化字段，且新增基于 PackedScene 快照的持久化节点分组读取、写入、撤销/重做和保存重开烟测（v0.63.0）
目标引擎：Godot 4.7+  
