...
状态：阶段 0 至阶段 7 的当前交付批次已完成。阶段 1 至阶段 5 已交付广泛的 2D/UI 场景、动画、物理、导航、TileMap、视觉、粒子与控件语义能力，以及受策略约束的项目 2D Resource 查询、创建、属性更新、保存和按资源隔离的撤销/重做；阶段 6 已交付场景启动/停止、游戏日志、真实运行时截图、键盘/鼠标/多点触摸输入模拟、项目 Input Map 动作与键盘/鼠标/手柄绑定编辑、独立 `Shortcut` 资源及其输入事件序列编辑、`AudioStreamPlayer2D` 运行时状态/播放/停止/定位控制、受控 CanvasItem 运行时 Tween、受限性能采样、客户端 PNG 内容断言和有总超时/自动清理边界的游戏测试编排；阶段 7 已交付运行时 `ClassDB` 2D 覆盖审计、完整快照、跨版本差异、类型详情反射和全允许节点生命周期验收；`OptionButton`/`MenuButton` 平面菜单写入已收紧为 Godot 场景真实可持久化字段，且新增基于 PackedScene 快照的持久化节点分组、元数据、Range 控件、文本输入控件、Label/RichTextLabel 文本展示，以及 ItemList 条目的文本、项目内图标、可选和禁用状态语义配置、撤销/重做与保存重开烟测（v0.68.0）
目标引擎：Godot 4.7+  
...
| `resource_get` / `resource_create` / `resource_set_properties` / `resource_save` / `resource_undo` / `resource_redo` | 查询、创建、修改、保存和撤销/重做受审计的项目 2D 资源 |
| `shortcut_get` / `shortcut_create` / `shortcut_set` / `shortcut_save` / `shortcut_undo` / `shortcut_redo` | 查询、创建、替换完整事件序列、保存和按 Shortcut 隔离的撤销/重做 |
...
- `shortcut_get`、`shortcut_create`、`shortcut_set`、`shortcut_save`、`shortcut_undo`、`shortcut_redo` 支持原生、独立项目 `Shortcut` 资源的完整事件序列工作流。它与 Input Map 复用完全一致的键盘、鼠标按钮、手柄按钮和手柄轴 JSON 协议；创建要求至少一个事件，更新可明确传递空数组以清空现有绑定，更新后需显式保存，撤销/重做只作用于指定资源。仅接受 `res://` 下独立 `.tres`/`.res`，拒绝导入或附带脚本的资源。`Shortcut.events` 是 Godot 对象数组，不能通过通用 Resource 属性写入器绕过此专用协议。
...
- `node_groups_get`、`node_group_add`、`node_group_remove` 支持节点的场景持久化组。读取使用 `PackedScene.pack(scene_root).get_state().get_node_groups()` 作为事实来源，避免 `Node.get_groups()` 混入运行时及 Godot 内部组；写入固定使用 `persistent=true`，仅允许当前场景拥有的节点，拒绝实例内部节点、`_` 前缀、控制字符、首尾空白和超过 128 字符的组名。遇到同名运行时组会明确拒绝，不会静默覆盖；新增、删除均进入场景 UndoRedo，且已验证复制后保存重开。
- `node_metadata_get`、`node_metadata_set`、`node_metadata_remove` 支持节点的场景持久化元数据。读取检查 PackedScene `SceneState` 中的 `metadata/<key>` 可存储属性，避免直接将编辑器内部 `_` 前缀数据暴露给 Agent；写入仅接受不以 `_` 开头、最多 128 字符的 ASCII 标识符，以及有限、深度不超过 8 层的非空 JSON 值，拒绝 Resource、Node、Script 与任意对象引用。写入仅允许当前场景拥有的节点，全部变更进入场景 UndoRedo，且已验证替换、删除、复制后保存重开。
- `resource_get`、`resource_create`、`resource_set_properties`、`resource_save`、`resource_undo`、`resource_redo` 支持当前 2D 覆盖目录中可安全管理的公开 Resource：可查询公开属性与类型元数据、在不存在的项目 `.tres`/`.res` 路径创建资源、按 Godot 属性声明进行严格 Variant 转换、在每个资源独立的 MCP UndoRedo 历史中原子更新，以及显式写盘。创建仅会在已验证的 `res://` 目标下补齐缺失父目录；导入资产、内嵌子资源、`Shader`、`ShaderMaterial` 和 `Shortcut` 不经该通用入口修改，继续走现有专用工作流，避免绕过类型或源码安全边界。
...
- `range_2d_get`、`range_2d_set`，支持 ProgressBar、HSlider、VSlider、HScrollBar、VScrollBar 和 SpinBox 的统一 Range 范围/值/步长/边界策略，以及各子类经 Godot 4.7 核验的持久化配置。批量写入会先验证完整目标状态，并以单个 UndoRedo 事务按安全顺序恢复允许范围、边界、步长和最终值，避免 setter 中间钳制破坏结果；ProgressBar 使用可读填充方向，Slider 使用可读 tick 位置，ScrollBar 的 `custom_step: -1` 保留 Godot 的使用 base step 语义。
- `text_input_2d_get`、`text_input_2d_set`，支持 LineEdit、TextEdit 与 CodeEdit 的经 Godot 4.7 核验的可持久化文本输入配置。LineEdit 覆盖单行文本、占位、输入/选择、虚拟键盘、光标、密码与 BiDi；TextEdit 覆盖多行文本、换行、滚动/小地图、光标、分词、显示与 BiDi；CodeEdit 追加缩进、补全、括号配对、折叠、行长度提示与 gutter 设置。写入在提交前严格校验枚举、文本、数组与 brace-pair 字典，并以单个 UndoRedo 事务恢复完整状态。运行时选择/滚动位置、脚本回调、SyntaxHighlighter 与其他对象资源不经此专用入口修改。
- `text_display_2d_get`、`text_display_2d_set`，支持 Label 与 RichTextLabel 的经 Godot 4.7 核验的可持久化文本展示配置。Label 覆盖文本、现有 LabelSettings 绑定、对齐、换行/trim、justification、截断、tab stops、可见字符动画与 BiDi；RichTextLabel 覆盖 BBCode、文本、fit/滚动/选择、对齐、换行/trim、justification、tab stops、链接下划线、线程显示延迟、可见字符动画与 BiDi。所有枚举和位标志采用可读稳定名称，tab stops 必须严格递增；LabelSettings 只可绑定现有项目资源，绝不通过该入口修改共享资源。RichTextEffect、脚本回调、结构化 BiDi 参数及运行时选择/滚动状态仍属后续工作。
- `item_list_items_get`、`item_list_items_set`、`item_list_items_clear`，支持 ItemList 条目的分页读取、完整替换及显式清空。仅暴露 Godot 4.7 场景实际序列化的 text、项目内 Texture2D icon、selectable 和 disabled 字段；完整替换通过一个 UndoRedo 事务提交并保留资源引用。运行时选择/焦点/滚动与未保存的 metadata、tooltip、颜色、图标区域、tag icon、翻译配置不经该入口写入。
- `control_get_styleboxes`、`control_stylebox_flat_upsert`、`control_stylebox_override_clear`，支持节点本地 `StyleBoxFlat` 状态覆盖及撤销/重做。
- `button_2d_get`、`button_2d_set`，支持所有 `BaseButton` 派生节点的 disabled/toggle/pressed/action/mask、外部 ButtonGroup/Shortcut 绑定和交互配置；`Button` 派生节点额外支持文本、图标、对齐、换行、文本方向和语言，`TextureButton` 支持五态贴图、BitMap 点击掩码、拉伸与翻转，`LinkButton` 支持 URI、下划线模式、溢出截断、截断字符、文本方向和语言。`button_menu_items_get`、`button_menu_items_set`、`button_menu_items_clear` 支持 OptionButton 的普通/分隔选择项与 MenuButton 的普通/复选/单选/分隔项；仅写入 Godot 场景可序列化的 `kind`、文本、ID、项目内图标、禁用状态，以及复选/单选的选中状态。平面菜单替换和清空均注册为单个撤销事务。嵌套 PopupMenu 子菜单、单项 Shortcut 与其他运行时菜单状态会明确拒绝写入，避免保存后丢失数据。
