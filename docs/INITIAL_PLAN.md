...
状态：阶段 0 已完成；阶段 1 已交付首批场景写入、结构编辑与信号管理能力（v0.4.0）
目标引擎：Godot 4.7+  
...
当前结构编辑拒绝跨 PackedScene 边界、包含不受支持 3D 节点的子树，以及需要修改外部动画资源的操作；删除会预检直接 `NodePath` 和动画轨道，避免留下悬空引用。信号工具仅操作场景内本地节点的持久化连接，并要求目标方法已存在，不会生成或修改脚本回调。这是为了避免 Agent 在无法证明安全的情况下静默破坏引用。资源编辑、自动脚本回调生成、语义化动画编辑和完整 PackedScene 工作流仍属于后续阶段。
