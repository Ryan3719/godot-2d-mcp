...
- `button_2d_get`、`button_2d_set`，支持所有 `BaseButton` 派生节点的 disabled/toggle/pressed/action/mask、外部 ButtonGroup/Shortcut 绑定和交互配置；`Button` 派生节点额外支持文本、图标、对齐、换行、文本方向和语言，`TextureButton` 支持五态贴图、BitMap 点击掩码、拉伸与翻转。`button_menu_items_get`、`button_menu_items_set`、`button_menu_items_clear` 支持 OptionButton 的普通/分隔选择项以及 MenuButton 的普通/复选/单选/多状态/分隔项、项目内图标、元数据、文本本地化、缩进与图标呈现；平面菜单替换和清空均注册为单个撤销事务。嵌套 PopupMenu 子菜单树和单项 Shortcut 资源仍会显式拒绝或不暴露写入，避免不完整序列化。
