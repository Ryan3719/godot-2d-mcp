...
The current `0.20.0` preview adds safe `Skeleton2D` and `Bone2D` authoring to scene, signal, animation, UI, Theme, collision, query, navigation, lighting, TileMap, viewport composition, and path editing. Agents can build valid bone chains and configure rest poses while retaining Godot-native undo and save behavior.
