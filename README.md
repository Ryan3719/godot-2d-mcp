...
- `range_2d_get` and `range_2d_set` for `ProgressBar`, `HSlider`, `VSlider`, `HScrollBar`, `VScrollBar`, and `SpinBox`. They expose a shared bounded range contract plus only the matching persistent concrete-control options.
- `text_input_2d_get` and `text_input_2d_set` for `LineEdit`, `TextEdit`, and `CodeEdit`. They expose only persistent, concrete text-input configuration with readable enums and strictly bounded text, list, and brace-pair payloads.
- `text_display_2d_get` and `text_display_2d_set` for `Label` and `RichTextLabel`. They expose persistent text-display settings with readable enums and flags, bounded text/tab-stop payloads, and safe existing-project `LabelSettings` binding.
- `item_list_items_get`, `item_list_items_set`, and `item_list_items_clear` for paginated, undoable persistent `ItemList` entry authoring. Each item exposes only the four fields Godot serializes in scenes: text, existing project-local icon, selectable state, and disabled state.
...
Use `range_2d_get` before `range_2d_set` on a `ProgressBar`, `HSlider`, `VSlider`, `HScrollBar`, `VScrollBar`, or `SpinBox`. Every type accepts `min_value`, `max_value`, `value`, `step`, `page`, `allow_greater`, `allow_lesser`, `exp_edit`, and `rounded`. The tool validates the complete resulting range before committing it: `page` cannot exceed the range, and without `allow_greater` the largest reachable value is `max_value - page`, matching Godot's native setter. `ProgressBar` adds readable `fill_mode`, indeterminate and percentage options; sliders add editable/scrollable and tick fields; scroll bars add `custom_step` (`-1` uses the base step); SpinBox adds alignment, arrow behavior, editable state, prefix/suffix, and text-update options. Call `scene_save` after a successful write to persist it.

Use `text_input_2d_get` before `text_input_2d_set` on a `LineEdit`, `TextEdit`, or `CodeEdit`. `LineEdit` supports text/placeholder, input and selection behavior, virtual-keyboard type, caret, secret, and bidirectional-text settings. `TextEdit` adds editable multiline text, wrapping, scroll/minimap, caret, word-separator, display, and bidirectional-text settings. `CodeEdit` adds persistent indentation, code completion, automatic brace pairs, folding, line-length guidelines, and gutter configuration. Every update snapshots the complete supported state and restores it in one normal scene undo transaction; text is limited to 65,536 characters and all code-editor lists and brace pairs are bounded. Runtime selection, scroll position, scripted completion/lookup callbacks, and `SyntaxHighlighter` resources remain outside this workflow. Call `scene_save` after a successful write to persist it.

Use `text_display_2d_get` before `text_display_2d_set` on a `Label` or `RichTextLabel`. `Label` supports text, existing project-local `LabelSettings` binding, alignment, wrapping/trimming, justification, truncation, tab stops, visible-character animation, and bidirectional-text settings. `RichTextLabel` supports BBCode, text, fit/scrolling/selection behavior, alignment, wrapping/trimming, justification, tab stops, link underlines, bounded threading display delay, visible-character animation, and bidirectional-text settings. Alignment, truncation, and visible-character modes use readable names; trim and justification flags use unique name lists. `label_settings_path` only binds an existing project-local `LabelSettings` resource and never changes that shared resource. Rich-text custom effects, scripted callbacks, structured BiDi parser options, and runtime selection/scroll state remain outside this workflow. Call `scene_save` after a successful write to persist it.

Use `item_list_items_get` before `item_list_items_set` on a local `ItemList`. A replacement takes a non-empty complete list of bounded entries with `text`, `icon_path`, `selectable`, and `disabled`; all fields except text are optional and use Godot defaults. `icon_path` must be an existing project-local `Texture2D` or an empty string to clear it. `item_list_items_clear` is the explicit destructive path for an empty list. Runtime selection, focus, scrolling, metadata, tooltip, colors, icon regions, tag icons, and translation overrides are intentionally outside this persistent scene workflow. Call `scene_save` after a successful write to persist it.
