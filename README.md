...
The current `0.18.0` preview adds safe 2D camera framing, parallax, and canvas-layer composition to scene, signal, animation, UI, Theme, collision, query, navigation, lighting, and TileMap editing. Agents can configure `Camera2D`, `Parallax2D`, `CanvasLayer`, and their scene-local `SubViewport` bindings with Godot-native undo and save behavior.
