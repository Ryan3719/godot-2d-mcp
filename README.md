...
The current `0.22.0` preview adds safe `GPUParticles2D` authoring to scene, signal, animation, UI, Theme, collision, query, navigation, lighting, TileMap, viewport composition, path, skeleton, and audio editing. Agents can configure persistent particle emission and drawing settings while retaining Godot-native undo and save behavior.
