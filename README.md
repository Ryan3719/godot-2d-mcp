...
Use `button_2d_get` before `button_2d_set` on any `BaseButton` descendant. `button_pressed: true` requires `toggle_mode: true`; `button_mask` uses `left`, `right`, and `middle`; `action_mode` uses `press` or `release`. `button_group_path`, `shortcut_path`, button icons, and `TextureButton` textures only bind existing project-local resources and can be cleared with an empty string. Text Button descendants include `Button`, `CheckButton`, `OptionButton`, and `MenuButton`; their popup-item collections remain a separate follow-up workflow.
