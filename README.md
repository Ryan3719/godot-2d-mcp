...
- `canvas_item_shader_get`, `canvas_item_shader_create`, `canvas_item_shader_bind`, `canvas_item_shader_set`, and `canvas_item_shader_clear` for safe `canvas_item` `ShaderMaterial` creation, binding, copy-on-write source replacement, and clearing on any local 2D `CanvasItem`.
