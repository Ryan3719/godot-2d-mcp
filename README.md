...
The current `0.17.0` preview adds safe scene-level 2D lighting and independent occluder authoring to scene, signal, animation, UI, Theme, collision, query, navigation, and TileMap editing. Agents can configure `PointLight2D`, `DirectionalLight2D`, and `LightOccluder2D` alongside TileSet physics, navigation, and occlusion data, with Godot-native undo and save behavior.
