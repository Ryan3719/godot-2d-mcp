...
The current `0.16.0` preview adds TileSet occlusion layers and Atlas TileData OccluderPolygon2D geometry to safe scene, signal, animation, UI, Theme, collision, query, navigation, and TileMap editing. Agents can define TileSet physics, navigation, and occlusion layers, typed custom data, terrains, atlas alternatives, per-tile terrain/custom data, collision polygons, NavigationPolygons, and occluders alongside atlas cells, with Godot-native undo and save behavior.
