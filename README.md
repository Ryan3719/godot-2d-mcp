...
The current `0.19.0` preview adds safe `Path2D` and `Curve2D` authoring to scene, signal, animation, UI, Theme, collision, query, navigation, lighting, TileMap, and viewport composition editing. Agents can configure Bézier control points and curve baking while retaining Godot-native undo and save behavior.
